...
impl Set {
    pub fn new(
        pool: Arc<Pool>, pipeline_layout: Arc<PipelineLayout>,
        buffer: Arc<UniformBuffer>) -> Self {
		...
		vulkan_check!(vk::vkAllocateDescriptorSets(
            pool.logical_device.vk_data, &alloc_info, &mut vk_data));
		...
		unsafe {
            vk::vkUpdateDescriptorSets(
                pool.logical_device.vk_data, 1, &write_descriptor_set, 0, null());
        }
        Set {
            pool: pool,
