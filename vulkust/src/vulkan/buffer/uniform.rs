...
use super::super::command::buffer::Buffer as CmdBuff;
...
impl Uniform {
    pub fn new(
        logical_device: Arc<LogicalDevice>, cmd_pool: Arc<CmdPool>, buffer_size: u32) -> Self {
		...
		vulkan_check!(vk::vkCreateBuffer(
            logical_device.vk_data, &buffer_info, null(), &mut buffer));
		...
		vulkan_check!(vk::vkAllocateMemory(
            logical_device.vk_data, &alloc_info, null(), &mut memory));
		vulkan_check!(vk::vkBindBufferMemory(logical_device.vk_data, buffer, memory, 0));
    ...
    pub fn update(&self, buffer_data: *const u8) {
		...
		vulkan_check!(vk::vkMapMemory(
            self.logical_device.vk_data, self.memory, 0, self.buffer_size, 0, &mut data));
        unsafe {
