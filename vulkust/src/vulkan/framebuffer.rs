...
impl Framebuffer {
    pub fn new(
        color_buffer: Arc<View>, depth_buffer: Arc<View>, render_pass: Arc<RenderPass>) -> Self {
        let attachments = vec![color_buffer.vk_data, depth_buffer.vk_data];
        let surface_caps =
            color_buffer.image.logical_device.physical_device.get_surface_capabilities();
        ...
        vulkan_check!(vk::vkCreateFramebuffer(
            color_buffer.image.logical_device.vk_data, &fb_create_info, null(), &mut vk_data));
        Framebuffer {
